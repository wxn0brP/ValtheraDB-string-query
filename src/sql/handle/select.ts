...
export function handleSelect(query: string, opts?: Opts): ValtheraQuery {
	const { mainQueryPart, whereClauseStr, groupClauseStr, dbFindOpts } =
		parseSelectClauses(query);

	const match = mainQueryPart.match(
		/SELECT\s+(.+?)\s+FROM\s+([\w/]+)((?:\s+JOIN\s+.+)*)?/i,
	);
	...
	if (joinPart && opts?.defaultDbKey) {
		...
		const relationsEngine = new JoinToRelationsEngine(
			opts.defaultDbKey,
			opts.tableDbMap,
		);
...
function parseSelectClauses(query: string): SelectClauses {
	...
	const limitIndex = findNumericClauseIndex(
		query,
		"LIMIT",
		offsetIndex === -1 ? query.length : offsetIndex,
	);
	const clauseIndexes = [
		whereIndex,
		groupIndex,
		...
		whereIndex !== -1
			? query
					.substring(
						...
						nextClauseIndex(query.length, whereIndex, [
							groupIndex,
							orderIndex,
							limitIndex,
							offsetIndex,
					...
					.trim()
			: undefined;
	const groupClauseStr =
		groupIndex !== -1
			? query
					.substring(
						groupIndex + "GROUP BY".length,
						nextClauseIndex(query.length, groupIndex, [
							orderIndex,
							limitIndex,
							offsetIndex,
							reverseIndex,
					...
					.trim()
			: undefined;
	const orderClauseStr =
		orderIndex !== -1
			? query
					.substring(
						orderIndex + "ORDER BY".length,
						nextClauseIndex(query.length, orderIndex, [
							limitIndex,
							offsetIndex,
							reverseIndex,
						]),
					...
					.trim()
			: undefined;
	const limitClauseStr =
		limitIndex !== -1
			? query
					.substring(
						limitIndex + "LIMIT".length,
						nextClauseIndex(query.length, limitIndex, [
							offsetIndex,
							reverseIndex,
						]),
					)
					.trim()
			: undefined;
	const offsetClauseStr =
		offsetIndex !== -1
			? query
					.substring(
						offsetIndex + "OFFSET".length,
						nextClauseIndex(query.length, offsetIndex, [
							reverseIndex,
						]),
					)
					.trim()
	...
	if (offsetClauseStr !== undefined) {
		dbFindOpts.offset = parseNonNegativeIntegerClause(
			"OFFSET",
			offsetClauseStr,
		);
	}
...
function findKeywordOutsideQuotes(
	...
	for (let i = start; i <= input.length - keyword.length; i++) {
		...
		if (
			upperInput.startsWith(upperKeyword, i) &&
...
function findNumericClauseIndex(
	input: string,
	keyword: "LIMIT" | "OFFSET",
	nextIndex: number = input.length,
): number {
	let searchStart = 0;
...
function findReverseClauseIndex(input: string): number {
	...
	while (searchStart < input.length) {
		const keywordIndex = findKeywordOutsideQuotes(
			input,
			"REVERSE",
			searchStart,
		);
...
function nextClauseIndex(
	fallback: number,
	currentIndex: number,
	indexes: number[],
) {
	const nextIndexes = indexes.filter(
...
function parseOrderClause(orderClause: string): DbFindOpts {
	...
	if (!match)
		throw new Error(
			`Invalid ORDER BY syntax near '${orderClause || "ORDER BY"}'`,
		);
...
function parseNonNegativeIntegerClause(
	name: "LIMIT" | "OFFSET",
	clause: string,
) {
...
function parseAggregateSelectClause(selectClause: string): DbFindOpts {
	...
	for (const part of splitSelectParts(selectClause)) {
		...
		const match = part.match(
			/^(MIN|MAX|AVG|COUNT)\(\s*([\w./]+)\s*\)(?:\s+AS\s+([\w.]+))?$/i,
		);
...
export function parseJoinClauses(joinPart: string): Record<string, string> {
	...
	const joinRegex =
		/\s+JOIN\s+([\w/]+)(?:\s+AS\s+)?([\w/]+)?\s+ON\s+([^\s]+)\s*=\s*([^\s]+)/gi;
...
export function parseSelectClause(selectClause: string): {
	select?: string[];
	exclude?: string[];
} {
...
function hasAggregateSelect(selectClause: string) {
	return splitSelectParts(selectClause).some(part =>
		/^(MIN|MAX|AVG|COUNT)\(/i.test(part),
	);
}

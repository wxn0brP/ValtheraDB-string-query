...
export function handleSelect(query, opts) {
    ...
    if (!match)
        throw new Error(formatSelectSyntaxError(mainQueryPart));
    ...
    return parseReturn("find", collection, {
        search: whereClause,
        findOpts,
        dbFindOpts,
    });
}
function parseSelectClauses(query) {
    ...
    const mainEnd = clauseIndexes.length
        ? Math.min(...clauseIndexes)
        : query.length;
    const mainQueryPart = query.substring(0, mainEnd).trim();
    const whereClauseStr = whereIndex !== -1
        ? query
            .substring(whereIndex + "WHERE".length, nextClauseIndex(query.length, whereIndex, [
            groupIndex,
            orderIndex,
            limitIndex,
            offsetIndex,
    ...
    const groupClauseStr = groupIndex !== -1
        ? query
            .substring(groupIndex + "GROUP BY".length, nextClauseIndex(query.length, groupIndex, [
            orderIndex,
            limitIndex,
            offsetIndex,
            reverseIndex,
    ...
    const orderClauseStr = orderIndex !== -1
        ? query
            .substring(orderIndex + "ORDER BY".length, nextClauseIndex(query.length, orderIndex, [
            limitIndex,
            offsetIndex,
            reverseIndex,
        ]))
    ...
    const limitClauseStr = limitIndex !== -1
        ? query
            .substring(limitIndex + "LIMIT".length, nextClauseIndex(query.length, limitIndex, [
            offsetIndex,
            reverseIndex,
        ]))
            .trim()
    ...
    const offsetClauseStr = offsetIndex !== -1
        ? query
            .substring(offsetIndex + "OFFSET".length, nextClauseIndex(query.length, offsetIndex, [
            reverseIndex,
        ]))
            .trim()
        : undefined;
...
function findKeywordOutsideQuotes(input, keyword, start = 0) {
    ...
    for (let i = start; i <= input.length - keyword.length; i++) {
        ...
        if (quote)
            continue;
        if (upperInput.startsWith(upperKeyword, i) &&
            hasKeywordBoundary(input, i, keyword.length)) {
            return i;
        }
    }
    return -1;
}
function findNumericClauseIndex(input, keyword, nextIndex = input.length) {
    let searchStart = 0;
    while (searchStart < input.length) {
        const keywordIndex = findKeywordOutsideQuotes(input, keyword, searchStart);
        if (keywordIndex === -1)
            return -1;
        const valuePart = input
            .substring(keywordIndex + keyword.length, nextIndex)
            .trim();
        if (/^[=<>!]/.test(valuePart)) {
...
function findReverseClauseIndex(input) {
    let searchStart = 0;
    while (searchStart < input.length) {
        ...
        if (valuePart &&
            !/^(LIMIT|OFFSET|ORDER\s+BY|GROUP\s+BY)\b/i.test(valuePart)) {
...
function hasKeywordBoundary(input, index, length) {
    const before = input[index - 1];
    const after = input[index + length];
    return (!before || /\s/.test(before)) && (!after || /\s/.test(after));
}
function nextClauseIndex(fallback, currentIndex, indexes) {
...
function parseOrderClause(orderClause) {
    ...
    if (!match)
        throw new Error(`Invalid ORDER BY syntax near '${orderClause || "ORDER BY"}'`);
    return {
        sortBy: match[1],
        sortAsc: match[2]?.toUpperCase() !== "DESC",
    };
}
function parseNonNegativeIntegerClause(name, clause) {
    if (!/^\d+$/.test(clause))
        throw new Error(`Invalid ${name} value`);
    return Number(clause);
}
function parseAggregateSelectClause(selectClause) {
    const dbFindOpts = {};
...
function parseGroupClause(groupClause) {
    const groupBy = groupClause.split(/\s*,\s*/).filter(Boolean);
    if (!groupBy.length)
        throw new Error("Invalid GROUP BY syntax near 'GROUP BY'");
    const invalidKey = groupBy.find(key => !/^[\w./]+$/.test(key));
    if (invalidKey)
        throw new Error(`Invalid GROUP BY key '${invalidKey}'`);
    return groupBy.length === 1 ? groupBy[0] : groupBy;
}
function formatSelectSyntaxError(query) {
    if (!/^SELECT\b/i.test(query))
        return `Invalid SELECT syntax near '${query}'`;
    if (!/\bFROM\b/i.test(query))
        return "Invalid SELECT syntax: missing FROM";
    const columns = query.match(/^SELECT\s*(.*?)\s+FROM\b/i)?.[1]?.trim();
    if (!columns)
        return "Invalid SELECT syntax: missing select list before FROM";
    const collection = query.match(/\bFROM\s*([^\s]*)/i)?.[1]?.trim();
    if (!collection)
        return "Invalid SELECT syntax: missing collection after FROM";
    if (!/^[\w/]+$/.test(collection))
        return `Invalid SELECT collection '${collection}'`;
    return `Invalid SELECT syntax near '${query}'`;
}
...
export function parseSelectClause(selectClause) {
    ...
    if (selectClause === "*")
        return {};
    if (hasAggregateSelect(selectClause))
        return {};
...
function hasAggregateSelect(selectClause) {
    return splitSelectParts(selectClause).some(part => /^(MIN|MAX|AVG|COUNT)\(/i.test(part));
}
function splitSelectParts(selectClause) {
    return selectClause
        .split(/\s*,\s*/)
        .map(part => part.trim())
        .filter(Boolean);

...
export function parseWhere(where) {
    ...
    for (let i = 0; i < tokens.length; i++) {
        ...
        if (token === "(") {
            ...
            if (opToken.toUpperCase() === "BETWEEN" ||
...
function formatConditionError(key, opToken, value) {
    const parts = [
        key,
        opToken,
        value,
        ...
        .join(" ");
    if (!key)
        return "Invalid WHERE condition: missing key";
    if (!opToken)
